/* @meta
{
  "name": "tampabay/get-article",
  "description": "Read Tampa Bay Times article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.tampabay.com",
  "args": {
    "url": { "required": true, "description": "Tampa Bay Times article URL (www.tampabay.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site tampabay/get-article https://www.tampabay.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "tampabay.com";
  var SITE_HOSTS = ["tampabay.com","www.tampabay.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Tampa Bay Times URL",
      hint: "Use an article link from www.tampabay.com.",
      action: "bun-browser open https://www.tampabay.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.tampabay.com in Chrome first, then retry.",
