/* @meta
{
  "name": "inc/get-article",
  "description": "Read inc.com article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.inc.com",
  "args": {
    "url": { "required": true, "description": "inc.com article URL (www.inc.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site inc/get-article https://www.inc.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "inc.com";
  var SITE_HOSTS = ["inc.com","www.inc.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a inc.com URL",
      hint: "Use an article link from www.inc.com.",
      action: "bun-browser open https://www.inc.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.inc.com in Chrome first, then retry.",
