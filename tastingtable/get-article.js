/* @meta
{
  "name": "tastingtable/get-article",
  "description": "Read Tasting Table article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.tastingtable.com",
  "args": {
    "url": { "required": true, "description": "Tasting Table article URL (www.tastingtable.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site tastingtable/get-article https://www.tastingtable.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "tastingtable.com";
  var SITE_HOSTS = ["tastingtable.com","www.tastingtable.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Tasting Table URL",
      hint: "Use an article link from www.tastingtable.com.",
      action: "bun-browser open https://www.tastingtable.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.tastingtable.com in Chrome first, then retry.",
