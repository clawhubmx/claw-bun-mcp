/* @meta
{
  "name": "bloomberg/get-article",
  "description": "Read Bloomberg article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.bloomberg.com",
  "args": {
    "url": { "required": true, "description": "Bloomberg article URL (www.bloomberg.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site bloomberg/get-article https://www.bloomberg.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["bloomberg.com","www.bloomberg.com"];
  ...
  async function waitForOpenPageArticle(url, attempts, delayMs) {
    for (var i = 0; i < attempts; i++) {
      ...
      if (isBotChallenge(document)) {
        await cf.waitForClearance({
          maxWaitMs: Math.max(delayMs * attempts * 4, 45000),
          pollMs: delayMs,
          autoClick: true,
          holdMs: 12000,
        });
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Bloomberg URL",
      hint: "Use an article link from www.bloomberg.com.",
      action: "bun-browser open https://www.bloomberg.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    if (!onArticlePage || isBotChallenge(document)) {
      await cf.waitForClearance({ url: raw, maxWaitMs: 45000, pollMs: 500, autoClick: true, holdMs: 12000 });
    }
    ...
    if (!resp.ok) {
      if ((resp.status === 403 || resp.status === 503) && isBotChallenge(document)) {
        var cfBypass = await cf.waitForClearance({ url: raw, maxWaitMs: 45000, pollMs: 500, autoClick: true, holdMs: 12000 });
        if (cfBypass.cleared) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Bloomberg bot check may require press-and-hold. Run: bun-browser site cloudflare/wait \"" + raw + "\" holdMs=12000 maxWaitMs=45000",
          action: "bun-browser site cloudflare/wait " + raw + " holdMs=12000 maxWaitMs=45000",
        };
      }
    }
    ...
    if (isBotChallenge(doc, html)) {
      var cfRetry = await cf.waitForClearance({ url: raw, maxWaitMs: 45000, pollMs: 500, autoClick: true, holdMs: 12000 });
      if (cfRetry.cleared) {
        resp = await fetch(fetchUrl, { credentials: "include", redirect: "follow" });
        html = await resp.text();
        doc = new DOMParser().parseFromString(html, "text/html");
        denied = detectAccessDenied(doc, html);
      }
    }

    if (denied || isBotChallenge(doc, html)) {
      return {
        error: denied ? "Access denied" : "Bot challenge blocked fetch",
        hint: "Bloomberg press-and-hold check did not clear. Run: bun-browser site cloudflare/wait \"" + raw + "\" holdMs=12000 maxWaitMs=45000",
        action: "bun-browser site cloudflare/wait " + raw + " holdMs=12000 maxWaitMs=45000",
