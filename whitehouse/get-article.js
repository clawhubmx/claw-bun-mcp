/* @meta
{
  "name": "whitehouse/get-article",
  "description": "Read The White House (.gov) article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.whitehouse.gov",
  "args": {
    "url": { "required": true, "description": "The White House (.gov) article URL (www.whitehouse.gov)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site whitehouse/get-article https://www.whitehouse.gov/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "whitehouse.gov";
  var SITE_HOSTS = ["whitehouse.gov","www.whitehouse.gov"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a The White House (.gov) URL",
      hint: "Use an article link from www.whitehouse.gov.",
      action: "bun-browser open https://www.whitehouse.gov",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.whitehouse.gov in Chrome first, then retry.",
