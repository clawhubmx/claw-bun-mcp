/* @meta
{
  "name": "pcmag/get-article",
  "description": "Read PCMag article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.pcmag.com",
  "args": {
    "url": { "required": true, "description": "PCMag article URL (www.pcmag.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site pcmag/get-article https://www.pcmag.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "pcmag.com";
  var SITE_HOSTS = ["pcmag.com","www.pcmag.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a PCMag URL",
      hint: "Use an article link from www.pcmag.com.",
      action: "bun-browser open https://www.pcmag.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.pcmag.com in Chrome first, then retry.",
