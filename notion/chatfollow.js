...
async function(args) {
  ...
  var answerFields = h.buildAnswerFields(waitFollowOut.answer, args.query, waitOpts);
  if (answerFields) {
    if (answerFields.answer != null) waitFollowOut.answer = answerFields.answer;
    if (answerFields.answerFormat) waitFollowOut.answerFormat = answerFields.answerFormat;
    if (answerFields.answerJson) waitFollowOut.answerJson = answerFields.answerJson;
    if (answerFields.jsonRecovered) waitFollowOut.jsonRecovered = true;
  } else {
    var parsedAnswerJson = h.parseAnswerJson(waitFollowOut.answer);
    if (parsedAnswerJson) {
      waitFollowOut.answerJson = parsedAnswerJson;
      waitFollowOut.answerFormat = 'json';
    }
  }
  var captureSource = h.getLastCaptureSource();
  if (captureSource) waitFollowOut.captureSource = captureSource;
  await h.closeModelPickerSurface();
  ...
  var answerFields = h.buildAnswerFields(out.answer, queryText, waitOpts);
  if (answerFields) {
    if (answerFields.answer != null) out.answer = answerFields.answer;
    if (answerFields.answerFormat) out.answerFormat = answerFields.answerFormat;
    if (answerFields.answerJson) out.answerJson = answerFields.answerJson;
    if (answerFields.jsonRecovered) out.jsonRecovered = true;
  } else {
    var parsedAnswerJson = h.parseAnswerJson(out.answer);
    if (parsedAnswerJson) {
      out.answerJson = parsedAnswerJson;
      out.answerFormat = 'json';
    }
  }
  var captureSource = h.getLastCaptureSource();
  if (captureSource) out.captureSource = captureSource;
