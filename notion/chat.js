...
async function(args) {
  ...
  var answerFields = h.buildAnswerFields(waitOut.answer, waitQuery, waitOpts);
  if (answerFields) {
    if (answerFields.answer != null) waitOut.answer = answerFields.answer;
    if (answerFields.answerFormat) waitOut.answerFormat = answerFields.answerFormat;
    if (answerFields.answerJson) waitOut.answerJson = answerFields.answerJson;
    if (answerFields.jsonRecovered) waitOut.jsonRecovered = true;
  } else {
    var parsedAnswerJson = h.parseAnswerJson(waitOut.answer);
    if (parsedAnswerJson) {
      waitOut.answerJson = parsedAnswerJson;
      waitOut.answerFormat = 'json';
    }
  }
  var captureSource = h.getLastCaptureSource();
  if (captureSource) waitOut.captureSource = captureSource;
