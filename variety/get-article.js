/* @meta
{
  "name": "variety/get-article",
  "description": "Read Variety article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.variety.com",
  "args": {
    "url": { "required": true, "description": "Variety article URL (www.variety.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site variety/get-article https://www.variety.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "variety.com";
  var SITE_HOSTS = ["variety.com","www.variety.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Variety URL",
      hint: "Use an article link from www.variety.com.",
      action: "bun-browser open https://www.variety.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.variety.com in Chrome first, then retry.",
