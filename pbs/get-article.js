/* @meta
{
  "name": "pbs/get-article",
  "description": "Read PBS article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.pbs.org",
  "args": {
    "url": { "required": true, "description": "PBS article URL (www.pbs.org)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site pbs/get-article https://www.pbs.org/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "pbs.org";
  var SITE_HOSTS = ["pbs.org","www.pbs.org"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a PBS URL",
      hint: "Use an article link from www.pbs.org.",
      action: "bun-browser open https://www.pbs.org",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.pbs.org in Chrome first, then retry.",
