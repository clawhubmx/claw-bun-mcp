/* @meta
{
  "name": "wired/get-article",
  "description": "Read WIRED article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.wired.com",
  "args": {
    "url": { "required": true, "description": "WIRED article URL (www.wired.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site wired/get-article https://www.wired.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "wired.com";
  var SITE_HOSTS = ["wired.com","www.wired.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a WIRED URL",
      hint: "Use an article link from www.wired.com.",
      action: "bun-browser open https://www.wired.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.wired.com in Chrome first, then retry.",
