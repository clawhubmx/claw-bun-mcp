/* @meta
{
  "name": "techspot/get-article",
  "description": "Read TechSpot article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.techspot.com",
  "args": {
    "url": { "required": true, "description": "TechSpot article URL (www.techspot.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site techspot/get-article https://www.techspot.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "techspot.com";
  var SITE_HOSTS = ["techspot.com","www.techspot.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a TechSpot URL",
      hint: "Use an article link from www.techspot.com.",
      action: "bun-browser open https://www.techspot.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.techspot.com in Chrome first, then retry.",
