/* @meta
{
  "name": "nascar/get-article",
  "description": "Read NASCAR.com article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.nascar.com",
  "args": {
    "url": { "required": true, "description": "NASCAR.com article URL (www.nascar.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site nascar/get-article https://www.nascar.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "nascar.com";
  var SITE_HOSTS = ["nascar.com","www.nascar.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a NASCAR.com URL",
      hint: "Use an article link from www.nascar.com.",
      action: "bun-browser open https://www.nascar.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.nascar.com in Chrome first, then retry.",
