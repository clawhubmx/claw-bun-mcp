/* @meta
{
  "name": "phys/get-article",
  "description": "Read Phys.org article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.phys.org",
  "args": {
    "url": { "required": true, "description": "Phys.org article URL (www.phys.org)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site phys/get-article https://www.phys.org/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "phys.org";
  var SITE_HOSTS = ["phys.org","www.phys.org"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Phys.org URL",
      hint: "Use an article link from www.phys.org.",
      action: "bun-browser open https://www.phys.org",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.phys.org in Chrome first, then retry.",
