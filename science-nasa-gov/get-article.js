/* @meta
{
  "name": "science-nasa-gov/get-article",
  "description": "Read NASA Science (.gov) article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "science.nasa.gov",
  "args": {
    "url": { "required": true, "description": "NASA Science (.gov) article URL (science.nasa.gov)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site science-nasa-gov/get-article https://science.nasa.gov/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "science.nasa.gov";
  var SITE_HOSTS = ["science.nasa.gov","www.science.nasa.gov"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a NASA Science (.gov) URL",
      hint: "Use an article link from science.nasa.gov.",
      action: "bun-browser open https://science.nasa.gov",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open science.nasa.gov in Chrome first, then retry.",
