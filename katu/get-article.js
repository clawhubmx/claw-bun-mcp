/* @meta
{
  "name": "katu/get-article",
  "description": "Read KATU article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.katu.com",
  "args": {
    "url": { "required": true, "description": "KATU article URL (www.katu.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site katu/get-article https://www.katu.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "katu.com";
  var SITE_HOSTS = ["katu.com","www.katu.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a KATU URL",
      hint: "Use an article link from www.katu.com.",
      action: "bun-browser open https://www.katu.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.katu.com in Chrome first, then retry.",
