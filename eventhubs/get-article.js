/* @meta
{
  "name": "eventhubs/get-article",
  "description": "Read EventHubs article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.eventhubs.com",
  "args": {
    "url": { "required": true, "description": "EventHubs article URL (www.eventhubs.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site eventhubs/get-article https://www.eventhubs.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["eventhubs.com","www.eventhubs.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a EventHubs URL",
      hint: "Use an article link from www.eventhubs.com.",
      action: "bun-browser open https://www.eventhubs.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      return {
        ...
        hint: "Article may be unavailable. Open www.eventhubs.com in Chrome first, then retry.",
