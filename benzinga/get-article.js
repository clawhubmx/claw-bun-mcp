/* @meta
{
  "name": "benzinga/get-article",
  "description": "Read Benzinga article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.benzinga.com",
  "args": {
    "url": { "required": true, "description": "Benzinga article URL (www.benzinga.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site benzinga/get-article https://www.benzinga.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "benzinga.com";
  var SITE_HOSTS = ["benzinga.com","www.benzinga.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Benzinga URL",
      hint: "Use an article link from www.benzinga.com.",
      action: "bun-browser open https://www.benzinga.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.benzinga.com in Chrome first, then retry.",
