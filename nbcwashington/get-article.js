/* @meta
{
  "name": "nbcwashington/get-article",
  "description": "Read NBC4 Washington article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.nbcwashington.com",
  "args": {
    "url": { "required": true, "description": "NBC4 Washington article URL (www.nbcwashington.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "banned": true,
  "banReason": "HTTP 403 — Akamai blocks programmatic fetch",
  "example": "bun-browser site nbcwashington/get-article https://www.nbcwashington.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "nbcwashington.com";
  ...
  var SITE_HOSTS = ["nbcwashington.com","www.nbcwashington.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a NBC4 Washington URL",
      hint: "Use an article link from www.nbcwashington.com.",
      action: "bun-browser open https://www.nbcwashington.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      if (resp.status === 403 || resp.status === 451) {
        return bannedResponse(raw, "HTTP " + resp.status + " — Akamai blocks programmatic fetch", state.title || null);
      }
      return {
        error: "HTTP " + resp.status,
        hint: "Article may be unavailable. Open www.nbcwashington.com in Chrome first, then retry.",
    ...
    if (denied || isBotChallenge(doc)) {
      return bannedResponse(raw, denied || "Bot challenge — HTTP 403 — Akamai blocks programmatic fetch", state.title || null);
    }
