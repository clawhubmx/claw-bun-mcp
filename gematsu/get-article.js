/* @meta
{
  "name": "gematsu/get-article",
  "description": "Read Gematsu article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.gematsu.com",
  "args": {
    "url": { "required": true, "description": "Gematsu article URL (www.gematsu.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site gematsu/get-article https://www.gematsu.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["gematsu.com","www.gematsu.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Gematsu URL",
      hint: "Use an article link from www.gematsu.com.",
      action: "bun-browser open https://www.gematsu.com",
    };
  }
  ...
  if (!state.articleBody && onArticlePage) {
    var directDom = extractDomBody(document);
    if (directDom) maybeSetBody(state, directDom, "openPageDirect");
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      if (resp.status === 403 || resp.status === 451) {
        return bannedResponse(raw, "HTTP " + resp.status + " — Cloudflare bot protection blocks programmatic access", state.title || null);
      }
      return {
        error: "HTTP " + resp.status,
        hint: "Article may be unavailable. Open www.gematsu.com in Chrome first, then retry.",
    ...
    if (denied || isBotChallenge(doc)) {
      return bannedResponse(raw, denied || "Bot challenge — Cloudflare bot protection blocks programmatic access", state.title || null);
    }
  ...
  if (!state.articleBody) {
    if (/just a moment/i.test(state.title || "") || detectAccessDenied(document, null)) {
      return bannedResponse(raw, "Cloudflare bot protection blocks programmatic access", state.title || null);
    }
    return {
