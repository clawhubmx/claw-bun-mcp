/* @meta
{
  "name": "yahoo/get-article",
  "description": "Read Yahoo article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.yahoo.com",
  "args": {
    "url": { "required": true, "description": "Yahoo article URL (www.yahoo.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site yahoo/get-article https://www.yahoo.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["yahoo.com","www.yahoo.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Yahoo URL",
      hint: "Use an article link from www.yahoo.com.",
      action: "bun-browser open https://www.yahoo.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      return {
        ...
        hint: "Article may be unavailable. Open www.yahoo.com in Chrome first, then retry.",
