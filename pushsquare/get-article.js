/* @meta
{
  "name": "pushsquare/get-article",
  "description": "Read pushsquare.com article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.pushsquare.com",
  "args": {
    "url": { "required": true, "description": "pushsquare.com article URL (www.pushsquare.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site pushsquare/get-article https://www.pushsquare.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "pushsquare.com";
  var SITE_HOSTS = ["pushsquare.com","www.pushsquare.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a pushsquare.com URL",
      hint: "Use an article link from www.pushsquare.com.",
      action: "bun-browser open https://www.pushsquare.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.pushsquare.com in Chrome first, then retry.",
