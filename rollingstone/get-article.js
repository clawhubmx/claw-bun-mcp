/* @meta
{
  "name": "rollingstone/get-article",
  "description": "Read Rolling Stone article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.rollingstone.com",
  "args": {
    "url": { "required": true, "description": "Rolling Stone article URL (www.rollingstone.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site rollingstone/get-article https://www.rollingstone.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "rollingstone.com";
  ...
  var SITE_HOSTS = ["rollingstone.com","www.rollingstone.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Rolling Stone URL",
      hint: "Use an article link from www.rollingstone.com.",
      action: "bun-browser open https://www.rollingstone.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      return {
        ...
        hint: "Article may be unavailable. Open www.rollingstone.com in Chrome first, then retry.",
