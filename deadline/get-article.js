/* @meta
{
  "name": "deadline/get-article",
  "description": "Read Deadline article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.deadline.com",
  "args": {
    "url": { "required": true, "description": "Deadline article URL (www.deadline.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site deadline/get-article https://www.deadline.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "deadline.com";
  var SITE_HOSTS = ["deadline.com","www.deadline.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Deadline URL",
      hint: "Use an article link from www.deadline.com.",
      action: "bun-browser open https://www.deadline.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.deadline.com in Chrome first, then retry.",
