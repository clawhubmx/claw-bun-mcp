/* @meta
{
  "name": "tmz/get-article",
  "description": "Read TMZ article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.tmz.com",
  "args": {
    "url": { "required": true, "description": "TMZ article URL (www.tmz.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site tmz/get-article https://www.tmz.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "tmz.com";
  var SITE_HOSTS = ["tmz.com","www.tmz.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a TMZ URL",
      hint: "Use an article link from www.tmz.com.",
      action: "bun-browser open https://www.tmz.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.tmz.com in Chrome first, then retry.",
