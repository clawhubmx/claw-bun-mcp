...
需要 Node.js 环境。安装后运行 `bun-browser --version` 验证。

### 2. 安装 Chrome 扩展

bun-browser 通过 Chrome 扩展与浏览器通信。首次运行 `bun-browser status` 会提示安装方式。安装扩展后确保已启用。

### 3. 安装 bb-sites adapter 库

```bash
# 从 fork 安装（包含本文档所需的全部脚本）
git clone -b feat/bilibili-opus https://github.com/shing19/bb-sites.git ~/.bun-browser/bb-sites

# 或者从上游安装（PR 合并后可用）
# git clone https://github.com/epiral/bb-sites.git ~/.bun-browser/bb-sites
```
...
```bash
mkdir -p ~/bin
chmod +x ~/.bun-browser/bb-sites/bilibili/user-videos.sh
ln -sf ~/.bun-browser/bb-sites/bilibili/user-videos.sh ~/bin/bb-user-videos
...
```bash
bun-browser status   # 检查是否运行
bun-browser start    # 如未运行则启动（前台模式）
```
...
```bash
bun-browser site bilibili/user-opus 582046218
```
...
```bash
bun-browser site bilibili/opus 1179671385624018945
```
...
```bash
bun-browser site bilibili/search "拉挖稀" --count 1
```
...
| `bilibili/_parse_user_videos.py` | 辅助脚本 | 解析 `bun-browser snapshot` 输出，提取视频标题、播放量、BV号 |
...
1. `bun-browser open` — 导航到用户空间页面
2. `bun-browser eval` — 点击"最多收藏"排序按钮，触发页面自带签名的 API 请求
3. `bun-browser snapshot` — 从渲染后的 DOM 提取视频数据

这个流程无法在单次 eval 中完成（导航会销毁 eval 上下文），所以封装成 shell 脚本。

### 其他踩坑记录

- **排序按钮无法通过 snapshot ref 点击**：页面上"最新发布/最多播放/最多收藏"渲染为纯文本节点，没有 ref。解决方法是 `bun-browser eval` 遍历 DOM 按文本内容匹配并 `.click()`。
...
- **`~/.bun-browser/bb-sites` 必须是 git 仓库**：如果手动 `cp` 文件进去，`bun-browser site update` 会失败。始终通过 `git clone` 安装。
