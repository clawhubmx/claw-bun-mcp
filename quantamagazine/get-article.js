/* @meta
{
  "name": "quantamagazine/get-article",
  "description": "Read Quanta Magazine article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.quantamagazine.org",
  "args": {
    "url": { "required": true, "description": "Quanta Magazine article URL (www.quantamagazine.org)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site quantamagazine/get-article https://www.quantamagazine.org/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "quantamagazine.org";
  var SITE_HOSTS = ["quantamagazine.org","www.quantamagazine.org"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Quanta Magazine URL",
      hint: "Use an article link from www.quantamagazine.org.",
      action: "bun-browser open https://www.quantamagazine.org",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.quantamagazine.org in Chrome first, then retry.",
