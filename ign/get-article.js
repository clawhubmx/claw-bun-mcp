/* @meta
{
  "name": "ign/get-article",
  "description": "Read IGN article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.ign.com",
  "args": {
    "url": { "required": true, "description": "IGN article URL (www.ign.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site ign/get-article https://www.ign.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "ign.com";
  var SITE_HOSTS = ["ign.com","www.ign.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a IGN URL",
      hint: "Use an article link from www.ign.com.",
      action: "bun-browser open https://www.ign.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.ign.com in Chrome first, then retry.",
