/* @meta
{
  "name": "consequence/get-article",
  "description": "Read Consequence of Sound article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.consequence.net",
  "args": {
    "url": { "required": true, "description": "Consequence of Sound article URL (www.consequence.net)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site consequence/get-article https://www.consequence.net/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["consequence.net","www.consequence.net"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Consequence of Sound URL",
      hint: "Use an article link from www.consequence.net.",
      action: "bun-browser open https://www.consequence.net",
    };
  }
  ...
  if (onArticlePage) {
    var openResult = await waitForOpenPageArticle(raw, 20, 400);
    if (openResult) {
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      return {
        ...
        hint: "Article may be unavailable. Open www.consequence.net in Chrome first, then retry.",
