/* @meta
{
  "name": "latimes/get-article",
  "description": "Read Los Angeles Times article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.latimes.com",
  "args": {
    "url": { "required": true, "description": "Los Angeles Times article URL (www.latimes.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site latimes/get-article https://www.latimes.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "latimes.com";
  var SITE_HOSTS = ["latimes.com","www.latimes.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Los Angeles Times URL",
      hint: "Use an article link from www.latimes.com.",
      action: "bun-browser open https://www.latimes.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.latimes.com in Chrome first, then retry.",
