/* @meta
{
  "name": "hayspost/get-article",
  "description": "Read Hays Post article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.hayspost.com",
  "args": {
    "url": { "required": true, "description": "Hays Post article URL (www.hayspost.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "banned": true,
  "banReason": "HTTP 403 — publisher blocks programmatic fetch",
  "example": "bun-browser site hayspost/get-article https://www.hayspost.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["hayspost.com","www.hayspost.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Hays Post URL",
      hint: "Use an article link from www.hayspost.com.",
      action: "bun-browser open https://www.hayspost.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      if (resp.status === 403 || resp.status === 451) {
        return bannedResponse(raw, "HTTP " + resp.status + " — publisher blocks programmatic fetch", state.title || null);
      }
      return {
        error: "HTTP " + resp.status,
        hint: "Article may be unavailable. Open www.hayspost.com in Chrome first, then retry.",
    ...
    if (denied || isBotChallenge(doc)) {
      return bannedResponse(raw, denied || "Bot challenge — HTTP 403 — publisher blocks programmatic fetch", state.title || null);
    }
