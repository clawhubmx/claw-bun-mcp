/* @meta
{
  "name": "statnews/get-article",
  "description": "Read statnews.com article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.statnews.com",
  "args": {
    "url": { "required": true, "description": "statnews.com article URL (www.statnews.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site statnews/get-article https://www.statnews.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["statnews.com","www.statnews.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a statnews.com URL",
      hint: "Use an article link from www.statnews.com.",
      action: "bun-browser open https://www.statnews.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      return {
        ...
        hint: "Article may be unavailable. Open www.statnews.com in Chrome first, then retry.",
