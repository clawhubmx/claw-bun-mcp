/* @meta
{
  "name": "ncronline/get-article",
  "description": "Read National Catholic Reporter article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.ncronline.org",
  "args": {
    "url": { "required": true, "description": "National Catholic Reporter article URL (www.ncronline.org)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site ncronline/get-article https://www.ncronline.org/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "ncronline.org";
  var SITE_HOSTS = ["ncronline.org","www.ncronline.org"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a National Catholic Reporter URL",
      hint: "Use an article link from www.ncronline.org.",
      action: "bun-browser open https://www.ncronline.org",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.ncronline.org in Chrome first, then retry.",
