...
{
  "name": "ew/get-article",
  "description": "Read Entertainment Weekly article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.ew.com",
  "args": {
    "url": { "required": true, "description": "Entertainment Weekly article URL (www.ew.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site ew/get-article https://www.ew.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "ew.com";
  var SITE_HOSTS = ["ew.com","www.ew.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Entertainment Weekly URL",
      hint: "Use an article link from www.ew.com.",
      action: "bun-browser open https://www.ew.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.ew.com in Chrome first, then retry.",
