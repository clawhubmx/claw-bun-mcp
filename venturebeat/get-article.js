/* @meta
{
  "name": "venturebeat/get-article",
  "description": "Read Venturebeat article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.venturebeat.com",
  "args": {
    "url": { "required": true, "description": "Venturebeat article URL (www.venturebeat.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site venturebeat/get-article https://www.venturebeat.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "venturebeat.com";
  var SITE_HOSTS = ["venturebeat.com","www.venturebeat.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Venturebeat URL",
      hint: "Use an article link from www.venturebeat.com.",
      action: "bun-browser open https://www.venturebeat.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.venturebeat.com in Chrome first, then retry.",
