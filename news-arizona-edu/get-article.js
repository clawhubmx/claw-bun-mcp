/* @meta
{
  "name": "news-arizona-edu/get-article",
  "description": "Read University of Arizona News article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "news.arizona.edu",
  "args": {
    "url": { "required": true, "description": "University of Arizona News article URL (news.arizona.edu)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site news-arizona-edu/get-article https://news.arizona.edu/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "news.arizona.edu";
  var SITE_HOSTS = ["news.arizona.edu","www.news.arizona.edu"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a University of Arizona News URL",
      hint: "Use an article link from news.arizona.edu.",
      action: "bun-browser open https://news.arizona.edu",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open news.arizona.edu in Chrome first, then retry.",
