/* @meta
{
  "name": "404media/get-article",
  "description": "Read 404 Media article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.404media.co",
  "args": {
    "url": { "required": true, "description": "404 Media article URL (www.404media.co)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site 404media/get-article https://www.404media.co/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "404media.co";
  var SITE_HOSTS = ["404media.co","www.404media.co"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a 404 Media URL",
      hint: "Use an article link from www.404media.co.",
      action: "bun-browser open https://www.404media.co",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.404media.co in Chrome first, then retry.",
