/* @meta
{
  "name": "newser/get-article",
  "description": "Read Newser article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.newser.com",
  "args": {
    "url": { "required": true, "description": "Newser article URL (www.newser.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "banned": true,
  "banReason": "HTTP 403 — publisher blocks programmatic fetch",
  "example": "bun-browser site newser/get-article https://www.newser.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["newser.com","www.newser.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Newser URL",
      hint: "Use an article link from www.newser.com.",
      action: "bun-browser open https://www.newser.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      ...
      return {
        ...
        hint: "Article may be unavailable. Open www.newser.com in Chrome first, then retry.",
