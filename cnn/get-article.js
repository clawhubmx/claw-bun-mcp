/* @meta
{
  "name": "cnn/get-article",
  "description": "Read CNN article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.cnn.com",
  "args": {
    "url": { "required": true, "description": "CNN article URL (www.cnn.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site cnn/get-article https://www.cnn.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "cnn.com";
  var SITE_HOSTS = ["cnn.com","www.cnn.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a CNN URL",
      hint: "Use an article link from www.cnn.com.",
      action: "bun-browser open https://www.cnn.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.cnn.com in Chrome first, then retry.",
