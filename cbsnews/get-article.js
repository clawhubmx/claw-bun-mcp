/* @meta
{
  "name": "cbsnews/get-article",
  "description": "Read CBS News article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.cbsnews.com",
  "args": {
    "url": { "required": true, "description": "CBS News article URL (www.cbsnews.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site cbsnews/get-article https://www.cbsnews.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "cbsnews.com";
  var SITE_HOSTS = ["cbsnews.com","www.cbsnews.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a CBS News URL",
      hint: "Use an article link from www.cbsnews.com.",
      action: "bun-browser open https://www.cbsnews.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.cbsnews.com in Chrome first, then retry.",
