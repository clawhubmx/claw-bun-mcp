/* @meta
{
  "name": "kvia/get-article",
  "description": "Read KVIA article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.kvia.com",
  "args": {
    "url": { "required": true, "description": "KVIA article URL (www.kvia.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site kvia/get-article https://www.kvia.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["kvia.com","www.kvia.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a KVIA URL",
      hint: "Use an article link from www.kvia.com.",
      action: "bun-browser open https://www.kvia.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      return {
        ...
        hint: "Article may be unavailable. Open www.kvia.com in Chrome first, then retry.",
