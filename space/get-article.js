/* @meta
{
  "name": "space/get-article",
  "description": "Read Space article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.space.com",
  "args": {
    "url": { "required": true, "description": "Space article URL (www.space.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site space/get-article https://www.space.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "space.com";
  var SITE_HOSTS = ["space.com","www.space.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Space URL",
      hint: "Use an article link from www.space.com.",
      action: "bun-browser open https://www.space.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.space.com in Chrome first, then retry.",
