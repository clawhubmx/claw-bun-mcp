/* @meta
{
  "name": "npr/get-article",
  "description": "Read NPR article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.npr.org",
  "args": {
    "url": { "required": true, "description": "NPR article URL (www.npr.org)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site npr/get-article https://www.npr.org/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "npr.org";
  var SITE_HOSTS = ["npr.org","www.npr.org"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a NPR URL",
      hint: "Use an article link from www.npr.org.",
      action: "bun-browser open https://www.npr.org",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.npr.org in Chrome first, then retry.",
