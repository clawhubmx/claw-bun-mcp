...
{
  "name": "dw/get-article",
  "description": "Read dw.com article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.dw.com",
  "args": {
    "url": { "required": true, "description": "dw.com article URL (www.dw.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site dw/get-article https://www.dw.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "dw.com";
  var SITE_HOSTS = ["dw.com","www.dw.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a dw.com URL",
      hint: "Use an article link from www.dw.com.",
      action: "bun-browser open https://www.dw.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.dw.com in Chrome first, then retry.",
