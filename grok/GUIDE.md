...
4. 用 `grok/chatfollow` 在同一对话中继续迭代 prompt
5. 用 `grok/search` 在历史对话中按关键词检索（可选）
...
grok/agent-chat          传入 agent id 或 project URL + prompt
      ↓
grok/chatfollow          用 conversationId 继续同一线程迭代 prompt
```
...
`answer` 即该 Agent 在 Project 上下文中的回复；`conversationId` 可传给 `grok/chatfollow` 继续同一会话。

## 继续对话 (grok/chatfollow)

在已有 Grok 对话中发送 follow-up prompt，保留完整上下文。适用于 prompt enhancer 等多轮迭代场景。

### 基本用法

```bash
# 第一轮：agent-chat 返回 conversationId
bun-browser site grok/agent-chat 2262a42d-da6b-478d-843f-69f811626817 "my dream is becoming a doctor"

# 第二轮及以后：用 conversationId 继续同一线程
bun-browser site grok/chatfollow 5c375edc-a6aa-40f5-bc7c-4f2c866c6b36 "make it more specific about medical school requirements"
bun-browser site grok/chatfollow 5c375edc-a6aa-40f5-bc7c-4f2c866c6b36 "now output only the final prompt, no explanation"
```

也支持完整对话 URL（`?rid=` 会被忽略）：

```bash
bun-browser site grok/chatfollow "https://grok.com/c/5c375edc-a6aa-40f5-bc7c-4f2c866c6b36" "refine the prompt"
```
...
`turn` 为当前对话中的 assistant 回复轮数，便于脚本跟踪迭代深度。
...
# 用返回的 conversationId 继续
bun-browser site grok/chatfollow 5c375edc-a6aa-40f5-bc7c-4f2c866c6b36 "continue refining the ACDQ prompt"
```

### 一键迭代脚本（可选）

```bash
AGENT_ID="2262a42d-da6b-478d-843f-69f811626817"
PROMPT="my dream is becoming a doctor"

CONV=$(bun-browser site grok/agent-chat "$AGENT_ID" "$PROMPT" --json --jq '.conversationId' -r)
bun-browser site grok/chatfollow "$CONV" "add constraints: 200 words, tone for a college advisor"
bun-browser site grok/chatfollow "$CONV" "now output only the final prompt"
```
...
2. 用 `grok/chatfollow <conversationId> "<prompt>"` 在原对话中继续提问（推荐）
3. 若需要在新 Project 线程中提问（不保留原上下文），可用 `results[].projects` 中的 `id` 调用 `grok/agent-chat`
4. 用 `results[].url` 在浏览器中手动查看原对话：
...
`conversation` 必须是 UUID 或 `https://grok.com/c/{uuid}` 格式。从 `grok/agent-chat` 返回的 `conversationId` 或 `grok/search` 结果中获取。
...
```json
{
  ...
  "action": "bun-browser open https://grok.com/c/<id>"
}
```

确认 `conversationId` 来自你自己的对话历史，且账号有访问权限。若刚创建对话，稍等片刻后重试。
