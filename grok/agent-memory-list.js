/* @meta
{
  ...
  "description": "列出 Agent Personal files (agent memory list: fileId, fileName, sizeBytes, mimeType, uploadTime)",
  "domain": "grok.com",
  "args": {
  ...
  "readOnly": true,
  "example": "bun-browser site grok/agent-memory-list 2262a42d-da6b-478d-843f-69f811626817"
}
*/

async function(args) {
  // API: GET /rest/assets?workspaceId={id}
  var helpers = (function installGrokMemoryHelpers() {
    if (globalThis.__grokMemoryHelpers && globalThis.__grokMemoryHelpers.fetchWorkspaceAssets) {
    ...
    async function readResponseError(resp) {
      ...
      return {
        ...
        hint: 'Grok API 请求失败，请确认已登录 grok.com',
        action: 'bun-browser open https://grok.com/'
      };
    }
    ...
    function toIsoTime(value) {
      ...
      if (typeof value === 'number') {
        var sec = value > 1e12 ? value : value * 1000;
    ...
    function normalizeAssetEntry(raw, agentId) {
      ...
      return {
        ...
        sizeBytes: raw.sizeBytes != null ? raw.sizeBytes : null,
        mimeType: raw.mimeType || raw.fileMimeType || raw.contentType || null,
        uploadTime: toIsoTime(raw.createTime || raw.createdAt || raw.uploadTime),
    ...
    function extractFilesFromAssets(assets, agentId) {
      ...
      return assets.map(function(entry) {
        return normalizeAssetEntry(entry, agentId);
      }).filter(Boolean);
    }

    async function fetchWorkspace(agentId) {
      var resp = await fetch('/rest/workspaces/' + agentId, {
        method: 'GET',
        credentials: 'include'
      });
    ...
    async function fetchWorkspaceAssets(agentId) {
      var resp = await fetch('/rest/assets?workspaceId=' + agentId, {
        method: 'GET',
        credentials: 'include'
      });
      ...
      try {
        ...
        return {
          error: 'Invalid JSON response',
          hint: 'Grok /rest/assets 返回格式异常，请刷新页面后重试',
          action: 'bun-browser open https://grok.com/'
        };
      }
    }
    ...
    async function uploadFileContent(fileName, mimeType, bytes) {
      ...
      if (!bytes || !bytes.length) {
        return { error: 'Empty file', hint: '文件内容为空，请提供有效的 content 或 fileBase64' };
      }
      if (bytes.length > maxBytes) {
        return {
          error: 'File too large',
          hint: '单文件最大约 48 MB，请拆分后重试'
        };
      }
      ...
      var resp = await fetch('/rest/app-chat/upload-file', {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          fileName: fileName,
          fileMimeType: mimeType || guessMimeType(fileName),
          content: base64,
      ...
      try {
        ...
        return {
          error: 'Invalid JSON response',
          hint: 'Grok /rest/app-chat/upload-file 返回格式异常',
          action: 'bun-browser open https://grok.com/'
        };
      }
      ...
      if (!data.fileMetadataId) {
        return {
          error: 'Upload failed',
          hint: '上传未返回 fileMetadataId，Grok API 可能已变更',
          action: 'bun-browser open https://grok.com/'
        };
      }
    ...
    async function createAndLinkAsset(agentId, uploadData) {
      var createResp = await fetch('/rest/assets?workspaceId=' + agentId, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          fileMetadataId: uploadData.fileMetadataId,
          name: uploadData.fileName,
          mimeType: uploadData.fileMimeType
        })
      });
      ...
      var asset;
      try {
        asset = await createResp.json();
      } catch (e) {
        return {
          error: 'Invalid JSON response',
          hint: 'Grok /rest/assets 返回格式异常',
          action: 'bun-browser open https://grok.com/'
        };
      }
      ...
      if (!asset.assetId) {
        return {
          ...
          hint: '创建 asset 未返回 assetId',
          action: 'bun-browser open https://grok.com/'
        };
      }

      var linkResp = await fetch('/rest/workspaces/' + agentId + '/assets', {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    async function removeAsset(agentId, assetId) {
      var resp = await fetch('/rest/assets/' + assetId + '?workspaceId=' + agentId, {
        method: 'DELETE',
        credentials: 'include'
      });
      ...
      if (resp.status === 404) {
        return {
          ...
          hint: '该 fileId 不在 Agent Personal files 中，请用 grok/agent-memory-list 查看',
    ...
    globalThis.__grokMemoryHelpers = {
      parseAgentId: parseAgentId,
      ensureLoggedIn: ensureLoggedIn,
      fetchWorkspace: fetchWorkspace,
      fetchWorkspaceAssets: fetchWorkspaceAssets,
      extractFilesFromAssets: extractFilesFromAssets,
      normalizeAssetEntry: normalizeAssetEntry,
      uploadFileContent: uploadFileContent,
      createAndLinkAsset: createAndLinkAsset,
  ...
  var workspace = wsResult.workspace;
  var files = helpers.extractFilesFromAssets(assetsResult.assets, agentId);
  var agentName = (workspace.name && String(workspace.name).trim()) || null;
  ...
  return {
    ...
    count: files.length,
    files: files,
    hint: files.length ? '移除文件: bun-browser site grok/agent-memory-remove ' + agentId + ' --fileId <id>' : '添加文件: bun-browser site grok/agent-memory-add ' + agentId + ' --fileName notes.md --content "..."'
  };
}
