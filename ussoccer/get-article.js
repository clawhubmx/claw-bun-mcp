/* @meta
{
  "name": "ussoccer/get-article",
  "description": "Read US Soccer article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.ussoccer.com",
  "args": {
    "url": { "required": true, "description": "US Soccer article URL (www.ussoccer.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "banned": true,
  "banReason": "Member-only content — US Soccer Insiders subscription required",
  "example": "bun-browser site ussoccer/get-article https://www.ussoccer.com/"
}
*/
...
async function (args) {
  ...
  var SITE_HOSTS = ["ussoccer.com","www.ussoccer.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a US Soccer URL",
      hint: "Use an article link from www.ussoccer.com.",
      action: "bun-browser open https://www.ussoccer.com",
    };
  }
  ...
  if (!state.articleBody || !state.title) {
    ...
    if (!resp.ok) {
      return {
        ...
        hint: "Article may be unavailable. Open www.ussoccer.com in Chrome first, then retry.",
  ...
  if (!state.articleBody) {
    if (/become a member|sign up to watch exclusive/i.test(document.body ? document.body.innerText : "")) {
      return bannedResponse(raw, "Member-only content — US Soccer Insiders subscription required", state.title || null);
    }
    return {
