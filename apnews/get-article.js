/* @meta
{
  "name": "apnews/get-article",
  "description": "Read AP News article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.apnews.com",
  "args": {
    "url": { "required": true, "description": "AP News article URL (www.apnews.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site apnews/get-article https://www.apnews.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "apnews.com";
  var SITE_HOSTS = ["apnews.com","www.apnews.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a AP News URL",
      hint: "Use an article link from www.apnews.com.",
      action: "bun-browser open https://www.apnews.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.apnews.com in Chrome first, then retry.",
