...
{
  "name": "{{COMMAND_NAME}}",
  "description": "Read {{PUBLISHER}} article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "{{META_DOMAIN}}",
  "args": {
    "url": { "required": true, "description": "{{PUBLISHER}} article URL ({{META_DOMAIN}})" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site {{COMMAND_NAME}} {{EXAMPLE_URL}}"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "{{ROOT_DOMAIN}}";
  var SITE_HOSTS = {{ALLOWED_HOSTNAMES}};
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a {{PUBLISHER}} URL",
      hint: "Use an article link from {{META_DOMAIN}}.",
      action: "bun-browser open https://{{META_DOMAIN}}",
    };
  }
