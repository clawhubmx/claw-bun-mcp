/* @meta
{
  "name": "nintendolife/get-article",
  "description": "Read Nintendo Life article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.nintendolife.com",
  "args": {
    "url": { "required": true, "description": "Nintendo Life article URL (www.nintendolife.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site nintendolife/get-article https://www.nintendolife.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "nintendolife.com";
  var SITE_HOSTS = ["nintendolife.com","www.nintendolife.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Nintendo Life URL",
      hint: "Use an article link from www.nintendolife.com.",
      action: "bun-browser open https://www.nintendolife.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.nintendolife.com in Chrome first, then retry.",
