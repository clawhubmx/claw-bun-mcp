/* @meta
{
  "name": "ca-news-yahoo-com/get-article",
  "description": "Read Yahoo News Canada article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "ca.news.yahoo.com",
  "args": {
    "url": { "required": true, "description": "Yahoo News Canada article URL (ca.news.yahoo.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site ca-news-yahoo-com/get-article https://ca.news.yahoo.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "ca.news.yahoo.com";
  var SITE_HOSTS = ["ca.news.yahoo.com","www.ca.news.yahoo.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Yahoo News Canada URL",
      hint: "Use an article link from ca.news.yahoo.com.",
      action: "bun-browser open https://ca.news.yahoo.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open ca.news.yahoo.com in Chrome first, then retry.",
