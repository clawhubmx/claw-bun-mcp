/* @meta
{
  "name": "vanityfair/get-article",
  "description": "Read Vanity Fair article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.vanityfair.com",
  "args": {
    "url": { "required": true, "description": "Vanity Fair article URL (www.vanityfair.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site vanityfair/get-article https://www.vanityfair.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "vanityfair.com";
  var SITE_HOSTS = ["vanityfair.com","www.vanityfair.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Vanity Fair URL",
      hint: "Use an article link from www.vanityfair.com.",
      action: "bun-browser open https://www.vanityfair.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.vanityfair.com in Chrome first, then retry.",
