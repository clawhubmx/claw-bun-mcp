/* @meta
{
  "name": "cpr/get-article",
  "description": "Read Colorado Public Radio article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.cpr.org",
  "args": {
    "url": { "required": true, "description": "Colorado Public Radio article URL (www.cpr.org)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site cpr/get-article https://www.cpr.org/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "cpr.org";
  var SITE_HOSTS = ["cpr.org","www.cpr.org"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a Colorado Public Radio URL",
      hint: "Use an article link from www.cpr.org.",
      action: "bun-browser open https://www.cpr.org",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.cpr.org in Chrome first, then retry.",
