/* @meta
{
  "name": "wtop/get-article",
  "description": "Read WTOP article title, author, date, and body. Uses JSON-LD NewsArticle metadata and generic article DOM extraction.",
  "domain": "www.wtop.com",
  "args": {
    "url": { "required": true, "description": "WTOP article URL (www.wtop.com)" }
  },
  "capabilities": ["network"],
  "readOnly": true,
  "example": "bun-browser site wtop/get-article https://www.wtop.com/"
}
*/
...
async function (args) {
  ...
  var SITE_ROOT = "wtop.com";
  var SITE_HOSTS = ["wtop.com","www.wtop.com"];
  ...
  if (!isSiteHostname(parsedUrl.hostname)) {
    return {
      error: "Not a WTOP URL",
      hint: "Use an article link from www.wtop.com.",
      action: "bun-browser open https://www.wtop.com",
    };
  }
    ...
    if (!resp.ok) {
      ...
      if (!resp.ok) {
        return {
          ...
          hint: "Article may be unavailable. Open www.wtop.com in Chrome first, then retry.",
